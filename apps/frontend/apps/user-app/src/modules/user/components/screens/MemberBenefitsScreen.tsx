...
const MemberBenefitsScreen: React.FC<Props> = ({ member, cart, vouchers, onSelectVoucher, onBack, onContinue }) => {
  ...
  return (
    <div className="w-full h-full bg-transparent flex flex-col overflow-hidden font-sans text-[#12201b]">
      {/* HEADER */}
      <header className="bg-white px-12 h-20 flex items-center justify-between border-b border-[#d9e2dc] sticky top-0 z-30">
        <button 
          onClick={onBack} 
          className="flex items-center gap-4 text-[#5f716a] hover:text-[#12201b] transition-all group active:scale-90"
        >
        ...
        <h1 className="text-xl font-black text-[#12201b] uppercase tracking-tight">Kupon Promo</h1>
      ...
      <div className="flex-1 overflow-y-auto p-12 no-scrollbar">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <section className="bg-white rounded-[2.5rem] p-8 px-10 border border-[#d9e2dc] shadow-[0_15px_50px_rgba(0,0,0,0.03)] flex items-center justify-between animate-in zoom-in duration-700">
             <div className="flex items-center gap-6">
                <div className="w-16 h-16 bg-[#fff2e9] rounded-2xl flex items-center justify-center text-[#e15b2d] shrink-0">
                   <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                     <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
                   </svg>
                </div>
                <div className="flex flex-col">
                   <p className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1.5 ${member.isAffiliate ? 'text-indigo-400' : 'text-[#5f716a]'}`}>
                      {member.isAffiliate ? 'AFFILIATE PARTNER' : 'PLATINUM MEMBER'}
                   </p>
                   <h3 className="text-3xl font-black text-[#12201b] tracking-tight leading-none uppercase">
                      {member.name}
                   </h3>
                </div>
             </div>
             ...
             <div className="text-right">
                 <p className="text-[10px] font-black text-[#5f716a] uppercase tracking-[0.2em] mb-1.5">SALDO POIN</p>
                <div className="flex items-baseline justify-end gap-1.5">
                   <span className="text-4xl font-black text-[#e15b2d] tracking-tighter leading-none">
          ...
          <section className="space-y-6 pb-20">
            <h3 className="text-sm font-black text-[#5f716a] uppercase tracking-widest mb-6 px-4">Daftar Voucher Tersedia</h3>
              ...
              {vouchers.length > 0 ? (
                vouchers.map((v) => (
                  <div 
                    ...
                    className="bg-white rounded-3xl border-2 border-[#d9e2dc] shadow-sm relative overflow-visible group cursor-pointer hover:border-[#e15b2d] hover:shadow-lg transition-all active:scale-[0.99]"
                  >
                    {/* Efek Gerigi Khas Voucher */}
                    <div className="absolute left-0 top-[68%] -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-white rounded-full border-2 border-[#d9e2dc] z-10" />
                    ...
                    <div className="p-8 pb-10 flex items-center justify-between">
                      <div className="flex-1 pr-6">
                        <div className="flex items-center gap-2 mb-3">
                           <span className="inline-block px-3 py-1 bg-[#e15b2d] text-white rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">Kupon Kiosk</span>
                        ...
                        <div className="w-24 h-24 bg-[#edf4ed] rounded-2xl flex flex-col items-center justify-center border border-[#d9e2dc] group-hover:bg-[#fff2e9] transition-colors shrink-0">
                    ...
                    <div className="p-6 px-8 flex items-center justify-between">
                      <p className="text-[10px] font-bold text-[#5f716a] uppercase tracking-wide">Promo Berlaku di Semua Menu</p>
                      <button className="bg-[#12201b] text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest group-hover:bg-[#e15b2d] transition-colors shadow-lg">
                        Gunakan Promo
                      </button>
                    </div>
                  </div>
